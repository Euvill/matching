  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if ( jacobians ) {
      ...
      if ( jacobians[0] ) {
        ...
        jacobian_i.block<3, 3>(0, 0) = ori_i.matrix().transpose() * (-1);
        jacobian_i.block<3, 3>(0, 3) = ori_i.matrix().transpose() * Sophus::SO3d::hat(pos_j - pos_i);
        jacobian_i.block<3, 3>(3, 0) = ori_i.matrix().transpose() * (-1);
        jacobian_i.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() * (-1);
      ...
      if ( jacobians[1] ) {
        ...
        jacobian_j.block<3, 3>(0, 0) = ori_i.matrix().transpose();
