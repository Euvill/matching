...
CeresSlidingWindow::CeresSlidingWindow(const int N) : kWindowSize(N) {   
    //
    // config optimizer:
    // 
    // a. loss function:
 ...
 */
void CeresSlidingWindow::AddPRVAGParam(const KeyFrame &lio_key_frame, const bool fixed) {
 ...
 * @return void
 */
void CeresSlidingWindow::AddPRVAGRelativePoseFactor(const int param_index_i, const int param_index_j, const Eigen::Matrix4d &relative_pose, const Eigen::VectorXd &noise) {
    ...
    // b.2. orientation, so3:
    residual_relative_pose.m.block<3, 1>(INDEX_R, 0) = Sophus::SO3d(Eigen::Quaterniond(relative_pose.block<3, 3>(0, 0).cast<double>())).log();
 ...
 * @return void
 */
void CeresSlidingWindow::AddPRVAGMapMatchingPoseFactor(const int param_index, const Eigen::Matrix4d &prior_pose, const Eigen::VectorXd &noise) {
    // create new residual block:
    ...
    // b.2. orientation, so3:
    residual_map_matching_pose.m.block<3, 1>(INDEX_R, 0) = Sophus::SO3d(Eigen::Quaterniond(prior_pose.block<3, 3>(0, 0).cast<double>())).log();
 ...
 * @return void
 */
void CeresSlidingWindow::AddPRVAGIMUPreIntegrationFactor(const int param_index_i, const int param_index_j, const IMUPreIntegrator::IMUPreIntegration &imu_pre_integration) {
    // create new residual block:
...
sliding_window::FactorPRVAGMapMatchingPose *CeresSlidingWindow::GetResMapMatchingPose(const CeresSlidingWindow::ResidualMapMatchingPose &res_map_matching_pose) {
...
sliding_window::FactorPRVAGRelativePose *CeresSlidingWindow::GetResRelativePose(const CeresSlidingWindow::ResidualRelativePose &res_relative_pose) {
...
sliding_window::FactorPRVAGIMUPreIntegration *CeresSlidingWindow::GetResIMUPreIntegration(const ResidualIMUPreIntegration &res_imu_pre_integration) {
...
bool CeresSlidingWindow::Optimize() {
    ...
    if (kWindowSize + 1 <= N) {
        ...
        for (int i = 1; i <= kWindowSize + 1; ++i) {
            ...
            // 用于用户显式传递参数模块，LocalParameterization参数，用于重构优化参数的维数
        ...
        if (!residual_blocks_.map_matching_pose.empty() && !residual_blocks_.relative_pose.empty() && !residual_blocks_.imu_pre_integration.empty()) {
            ...
            factor_marginalization->SetResMapMatchingPose(  factor_map_matching_pose,   std::vector<double *>{key_frame_m.prvag});
            factor_marginalization->SetResRelativePose(     factor_relative_pose,       std::vector<double *>{key_frame_m.prvag, key_frame_r.prvag});
            factor_marginalization->SetResIMUPreIntegration(factor_imu_pre_integration, std::vector<double *>{key_frame_m.prvag, key_frame_r.prvag});
        ...
        if (!residual_blocks_.map_matching_pose.empty()) {
            for ( const auto &residual_map_matching_pose: residual_blocks_.map_matching_pose) {
                ...
                sliding_window::FactorPRVAGMapMatchingPose *factor_map_matching_pose = GetResMapMatchingPose(residual_map_matching_pose);
        ...
        if (!residual_blocks_.relative_pose.empty()) {
            for ( const auto &residual_relative_pose: residual_blocks_.relative_pose) {
                ...
                sliding_window::FactorPRVAGRelativePose *factor_relative_pose = GetResRelativePose(residual_relative_pose);
        ...
        if (!residual_blocks_.imu_pre_integration.empty()) {
            for (const auto &residual_imu_pre_integration: residual_blocks_.imu_pre_integration) {
                ...
                sliding_window::FactorPRVAGIMUPreIntegration *factor_imu_pre_integration = GetResIMUPreIntegration(residual_imu_pre_integration);
