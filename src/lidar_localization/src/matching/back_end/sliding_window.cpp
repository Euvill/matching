...
bool SlidingWindow::MaybeNewKeyFrame(const PoseData &laser_odom, const PoseData &map_matching_odom, IMUData &imu_data, const PoseData &gnss_odom) {
    static KeyFrame last_key_frame;

   /* std::cout << std::endl;
    std::cout << "bias accel: " <<
                 imu_data.accel_bias.x << " " << 
                 imu_data.accel_bias.y << " " << 
                 imu_data.accel_bias.z << " " << std::endl;
    std::cout << std::endl;*/
    // 
    ...
    if (key_frames_.lidar.empty()) {
        ...
        if (imu_pre_integrator_ptr_) {
            ...
            imu_data.accel_bias.x = last_key_frame.bias.accel(0);
            imu_data.accel_bias.y = last_key_frame.bias.accel(1);
            imu_data.accel_bias.z = last_key_frame.bias.accel(2);

            imu_data.gyro_bias.x = last_key_frame.bias.gyro(0);
            imu_data.gyro_bias.y = last_key_frame.bias.gyro(1);
            imu_data.gyro_bias.z = last_key_frame.bias.gyro(2);
...
bool SlidingWindow::Update(void) {
    ...
    /*std::cout << std::endl;
    std::cout << "bias accel: " <<
                 current_key_frame_.bias.accel(0) << " " << 
                 current_key_frame_.bias.accel(1) << " " << 
                 current_key_frame_.bias.accel(2) << " " << std::endl;
    std::cout << std::endl;*/
